...
void mon1_setup(void)
{
    Serial1.begin(9600);
    Serial1.println("Welcome! This is the Serial1 debug console.");
}
...
void mon1_loop()
{
    ...
    while (true) {
        ...
        if (line_end == false) {        
            ...
            // default for Serial1 is ECHO_OFF, and there doesn't seem to be a way to change that
            Serial1.print(c);
        }
        else {
    ...
    if (commandIndex == -1) {
        Serial1.print("The command `");
        Serial1.print(cmdstr);
        Serial1.println("' was not recognized.");
        return;
    }

    if (!_consoleCommandTable[commandIndex].handler) {
        Serial1.println("That command has not yet been implemented.");
        return;
    }
    ...
    switch (returnVal) {
        ...
        default:
            Serial1.println("Warning: unknown return value!");
            break;
    }
}
...
static int consoleHandler_help(const char *line){
    ...
    if (!*line) {
        ...
        for (i=0; _consoleCommandTable[i].name; i++) {
            ...
            Serial1.print("  ");
            Serial1.print(_consoleCommandTable[i].name);
            Serial1.print("\t  ");
            Serial1.println(_consoleCommandTable[i].description);
        }
    } else {
...
static void doRepeat(RepeatFunc func, uint32_t arg0, uint32_t arg1, uint32_t arg2, uint32_t arg3)
{
    ...
    Serial1.read(); /* remove char from input buf */
}
...
static int consoleHandler_dw(const char *line)
{
    ...
    Serial1.print("Calling digitalWrite(");
    Serial1.print(pin);
    Serial1.print(", ");
    Serial1.print(val);
    Serial1.println(").");
...
static int consoleHandler_aw(const char *line)
{
    ...
    uint32_t pin = strtol(line, &endptr, 10);
    int32_t val = strtol(endptr, NULL, 0);
    Serial1.print("Calling analogWrite(");
    Serial1.print(pin);
    Serial1.print(", ");
    Serial1.print(val);
    Serial1.println(").");
...
static int consoleHandler_dm(const char *line)
{
    ...
    if (len <= 0) {
        Serial1.println("Bad number of bytes to dump.");
        return RETURN_FAIL_PRINT_USAGE;
    }
...
static int consoleHandler_spi(const char *line)
{
    ...
    Serial1.print("Calling SPI.transfer(");
    if (cs) {
        Serial1.print(cs);
        Serial1.print(", ");
    }
...
static void printUtilization()
{
    ...
    Serial1.write("Total CPU Load: ");
    Serial1.print(idleLoadInt);
    Serial1.print(".");
    Serial1.println(idleLoadFrac);
    Serial1.println("");
    
    /* collect stats on all statically Created tasks */
    Serial1.println("Task info:");
    ...
    Serial1.print("Hwi stack usage: ");
    Serial1.print(hwiStackStat.hwiStackPeak);
    Serial1.print("/");
    Serial1.println(hwiStackStat.hwiStackSize);
    Serial1.println("");
    
    Memory_getStats(NULL, &memStat);
    Serial1.print("Heap usage: ");
    Serial1.print(memStat.totalSize - memStat.totalFreeSize);
    Serial1.print("/");
    Serial1.println(memStat.totalSize);
}
...
static int consoleHandler_pri(const char *line)
{
    ...
    if ((pri < -1) || (pri >= (int)Task_numPriorities)) {
        Serial1.println("Invalid priority!");
        return RETURN_FAIL_PRINT_USAGE;
    }
